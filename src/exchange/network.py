...
class ProxyDetector:
    ...
    for external APIs (like Binance).
    """
    ...
    def __init__(self, test_url: str = 'https://api.binance.com/api/v3/ping', timeout: int = 5):
        ...
        Args:
            test_url: URL to test proxy connections
    ...
    def detect_proxy(self, check_env: bool = True, check_system: bool = True, 
        ...
        # 3. Check requests library auto-detection
        requests_proxies = self._check_requests_proxy()
        if requests_proxies:
            proxy_sources.append(("Requests Auto-detected", requests_proxies))
    ...
    def _check_system_proxy(self) -> Optional[Dict[str, str]]:
        """Check system proxy configuration using platform-specific methods"""
        proxies = None
    ...
    def _get_linux_proxy(self) -> Optional[Dict[str, str]]:
        """Get proxy settings from Linux environment"""
    ...
    def _check_requests_proxy(self) -> Optional[Dict[str, str]]:
        """Check proxy settings using requests library auto-detection"""
        try:
            ...
            logger.debug(f"Error checking requests proxy: {e}")
    ...
    def test_proxy(self, proxy_settings: Dict[str, str]) -> bool:
        """
        Test if a proxy configuration works for the target API
        ...
        try:
            ...
            logger.debug(f"Proxy test {'successful' if success else 'failed'} with status code {response.status_code}")
            return success
        except Exception as e:
...
def detect_proxy(test_url: str = 'https://api.binance.com/api/v3/ping') -> Optional[Dict[str, str]]:
    """
    Convenience function to detect available proxy settings for target URL
...
def test_connection(url: str = 'https://api.binance.com/api/v3/ping', 
                   ...
                   timeout: int = 5) -> bool:
    """
    Test connection to URL, optionally using a proxy
