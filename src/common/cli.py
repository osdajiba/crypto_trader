...
class CliProgressBar:
    """Command line progress bar for displaying trading system progress"""
    ...
    def __init__(self, total=100, desc="Progress"):
        ...
        Args:
            ...
            desc: Description displayed alongside progress bar
        """
...
def setup_progress_display(verbosity_level=0):
    """Setup the appropriate progress display based on verbosity
    
    Args:
        verbosity_level: Level of detail (0=normal, 1+=more verbose)
        
    Returns:
        Progress display handler
    """
    if verbosity_level >= 2:
        # For high verbosity, use detailed output
        return DetailedProgressDisplay()
    else:
        # Default to progress bar
        return CliProgressBar(desc="Trading Pipeline")


class DetailedProgressDisplay:
    """Text-based progress display for verbose output"""
    ...
    def update(self, percent, message):
        """Print progress updates when they change significantly
        
        Args:
            percent: Current progress percentage (0-100)
            message: Status message to display
        """
        # Only print updates when percentage changes by 5% or message changes
        if (int(percent) - self.last_percent >= 5) or (message != self.last_message):
